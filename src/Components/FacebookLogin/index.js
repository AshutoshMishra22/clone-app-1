...
import { EMAIL, PASSWORD } from '../../constant/formFields';
...
import { clearFormData } from '../../actions/actionCreators';
const LoginFb = (props) => {
	const { testReducer, handleChange, clearFormData } = props;
	const [open, setOpen] = useState(false);
	const [showPassword, setShowPassword] = useState(false);
	let emailErr = testReducer[EMAIL].err;
	let passErr = testReducer[PASSWORD].err;
	const handleClose = () => {
		setOpen(false);
		clearFormData();
	};
			...
			<div className="loginBoxContainer">
				<div className="loginBoxContent">
					...
					>
						...
						<FormControl sx={{ m: 1, width: '25ch' }} variant="outlined">
							<OutlinedInput
								...
								value={testReducer[PASSWORD].value}
					...
					<Button
						...
						disabled={
							testReducer[EMAIL].value === '' ||
							testReducer[PASSWORD].value === '' ||
							emailErr.length > 0 ||
							passErr.length > 0
						}
					>
